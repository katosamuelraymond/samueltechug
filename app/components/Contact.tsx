          ...
          >
            {[
              ...
              <div key={item.label} className="flex items-start gap-4">
                <div className="mt-0.5 w-10 h-10 flex-shrink-0 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400">
                  {item.icon}
                </div>
                <div>
                  <p className="text-xs text-zinc-500 uppercase tracking-wide font-medium">{item.label}</p>
                  {item.href ? (
                    <a href={item.href} target="_blank" rel="noopener noreferrer" className="text-zinc-300 hover:text-orange-400 transition-colors text-sm break-all">
                      {item.value}
                    </a>
                  ) : (
